...
class DSLParser:
    ...
    def parse(self, block: Block) -> None:
        ...
        self.do_post_block_processing_cleanup(function)
    ...
    def parse_parameter(self, function: Function, line: str) -> None:
        ...
        if not module:
            fail(
                f"Function {function.name} has an invalid parameter declaration:\n\t"
                f"{line}"
            )
    ...
    def do_post_block_processing_cleanup(self, function: Function | None) -> None:
        """
        ...
        if self.keyword_only:
            values = function.parameters.values()
            if not values:
            ...
            if no_parameter_after_star:
                fail(
                    f"Function {function.name} specifies '*' "
                    f"without any parameters afterwards."
                )
