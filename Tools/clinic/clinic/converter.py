...
import clinic
from clinic import fail
from clinic import Sentinels, unspecified, unknown
from clinic.crenderdata import CRenderData, Include, TemplateDict
from clinic.function import Function, Parameter
...
class CConverter(metaclass=CConverterAutoRegister):
    ...
    def __init__(self,
        ...
        self.name = clinic.ensure_legal_c_identifier(name)
    ...
    def parse_argument(self, args: list[str]) -> None:
        ...
        if self.encoding:
            args.append(clinic.c_repr(self.encoding))
    ...
    def parser_name(self) -> str:
        if self.name in clinic.CLINIC_PREFIXED_ARGS: # bpo-39741
            return clinic.CLINIC_PREFIX + self.name
        else:
            return self.name
